...
?>
    ...
    <div style="margin: 0 auto 0 auto; width: 80%;" class="mx-auto">
        <?php
        $hats = $db->req_sql("SELECT * FROM hats ORDER BY id DESC LIMIT ".(!isset($_GET['p']) ? 0 : $_GET['p'] * 30 - 30).",30", [], true);
        if($hats == null) {die("ERR");}
        while ($hat = $hats->fetch_assoc()) {

            $html->card("/home/pages/hat?id=" . $hat['id'], "https://www.roblox.com/asset-thumbnail/image?assetId=" . $hat['asset'] . "&width=420&height=420&format=png", "(" . ($hat['cost'] == '-1' ? "offsale" : $hat['cost']) . ")<br>" . $hat['name']);
        }
        ?>
    </div>
    <br>
        ...
        <nav>
            <ul class="pagination justify-content-center">
                <li class="page-item"><a class="page-link" href="?p=<?php
                    ...
                    ?>">Previous</a></li>
                <?php
                $count = $db->req_sql("SELECT id FROM hats",[],true)->num_rows;
