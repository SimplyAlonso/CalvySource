...
$hat = $db->req_sql("SELECT * FROM tshirts WHERE id = ?",[$_POST['id']]);
if($hat == null) {die("error");}
if($hat['cost'] == '-1') {die("Not for sale");}
$isowned = $db->req_sql("SELECT * FROM ownedassets WHERE user = ? AND assettype = 'tshirt' AND assetid = ?",[$user->userid(),$_POST['id']]);
...
$db->req_sql("INSERT INTO ownedassets (user,assettype,assetid) VALUES (?,?,?)", [$user->userid(),"tshirt",$_POST['id']]);
die("true");
?>